...
// reapplyReverseAfterResets: empty string returns empty.
func TestReapplyReverseAfterResets_Empty(t *testing.T) {
...
// joinWrappedLines: empty slice returns empty string.
func TestJoinWrappedLines_Empty(t *testing.T) {
...
// detectSideContentCols: gutterCol <= 0 returns (-1, -1) immediately.
func TestDetectSideContentCols_NonPositiveGutter(t *testing.T) {
	left, right := detectSideContentCols("any content", 0)
	if left != -1 || right != -1 {
		t.Fatalf("expected (-1, -1) for gutterCol=0, got (%d, %d)", left, right)
	}
	left, right = detectSideContentCols("any content", -5)
	if left != -1 || right != -1 {
		t.Fatalf("expected (-1, -1) for gutterCol=-5, got (%d, %d)", left, right)
	}
}

// clampToLine: a > lineWidth — when the start column is past the line width
// both a and b get clamped to lineWidth, and if a > b after clamping, a = b.
func TestClampToLine_AExceedsLineWidth(t *testing.T) {
	// a > lineWidth: a gets clamped to lineWidth
	a, b := clampToLine(50, 60, 40)
	if a != 40 || b != 40 {
		t.Fatalf("expected (40, 40), got (%d, %d)", a, b)
	}

	// a > lineWidth, b < lineWidth: a gets clamped, then a > b so a = b
	a, b = clampToLine(50, 30, 40)
	if a != 30 || b != 30 {
		t.Fatalf("expected (30, 30) when a>lineWidth and b<lineWidth, got (%d, %d)", a, b)
	}
}

// clampToBand: a > b after clamping — when a is clamped upward past b.
func TestClampToBand_AExceedsBAfterClamp(t *testing.T) {
	// a=50 gets clamped to lo=10, b=5 stays (5 < 20), then a(10) > b(5): a = b = 5
	a, b := clampToBand(50, 5, [2]int{10, 20})
	if a != 5 || b != 5 {
		t.Fatalf("expected (5, 5) when a clamped past b, got (%d, %d)", a, b)
	}
}

// reapplyReverseAfterResets: non-SGR escape sequences (e.g. OSC) are passed
// through unchanged without re-asserting reverse.
func TestReapplyReverseAfterResets_NonSGREscape(t *testing.T) {
	// OSC sequence: \x1b]0;title\x1b\\ — the ] is not '[', so it hits the non-CSI path
	input := "before\x1b]0;title\x1b\\after"
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected non-SGR escape preserved verbatim, got %q", got)
	}
}

// reapplyReverseAfterResets: SGR that does NOT clear reverse (e.g. bold \x1b[1m)
// is written through without appending \x1b[7m.
func TestReapplyReverseAfterResets_SGRNoClearReverse(t *testing.T) {
	input := "\x1b[1m" // bold — sgrClearsReverse("1") returns false
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected bold SGR preserved without re-assert, got %q", got)
	}

	// Multiple SGRs: only the resetting one gets a re-assert.
	mixed := "\x1b[1m\x1b[0m\x1b[31m"
	got = reapplyReverseAfterResets(mixed)
	// \x1b[1m → no re-assert
	// \x1b[0m → re-assert (clears reverse)
	// \x1b[31m → no re-assert
	if !strings.Contains(got, "\x1b[0m\x1b[7m") {
		t.Fatalf("expected re-assert after \\x1b[0m, got %q", got)
	}
	if strings.Contains(got, "\x1b[1m\x1b[7m") {
		t.Fatalf("expected no re-assert after bold, got %q", got)
	}
}

// reapplyReverseAfterResets: incomplete CSI escape at end of string is written
// through as-is (no terminator found, end >= len(s)).
func TestReapplyReverseAfterResets_IncompleteEscape(t *testing.T) {
	input := "text\x1b["
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected incomplete escape preserved, got %q", got)
	}

	// Partial params without terminator
	input = "text\x1b[31"
	got = reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected incomplete params preserved, got %q", got)
	}
}

// reapplyReverseAfterResets: bare ESC at end of string (i+1 >= len(s)).
func TestReapplyReverseAfterResets_BareEscapeAtEnd(t *testing.T) {
	input := "text\x1b"
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected bare ESC preserved, got %q", got)
	}
}

// reapplyReverseAfterResets: ESC followed by non-[ byte (not CSI, e.g. OSC \x1b]).
func TestReapplyReverseAfterResets_EscNonCSI(t *testing.T) {
	// \x1b followed by ] (beginning of OSC sequence)
	input := "text\x1b]0;title\x07more"
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected non-CSI escape byte preserved, got %q", got)
	}
}

// reapplyReverseAfterResets: CSI terminator that is NOT 'm' (e.g. cursor move \x1b[A)
// is written through without re-asserting reverse.
func TestReapplyReverseAfterResets_CSITerminatorNotM(t *testing.T) {
	input := "\x1b[A" // cursor up — terminator is 'A' (0x41 ∈ [0x40, 0x7e])
	got := reapplyReverseAfterResets(input)
	if got != input {
		t.Fatalf("expected CSI non-m sequence preserved, got %q", got)
	}
	if strings.Contains(got, "\x1b[7m") {
		t.Fatalf("expected no reverse re-assert for non-m CSI")
	}
}

// sgrClearsReverse: non-matching parameter values like "1" (bold), "7" (reverse-set),
// "31" (red fg) return false.
func TestSgrClearsReverse_NonMatchingParams(t *testing.T) {
	cases := []struct {
		params string
		want   bool
	}{
		{"1", false},  // bold
		{"7", false},  // reverse SET (not clear)
		{"31", false}, // red foreground
		{"1;7", false},
		{"31;1", false},
		// Matching values still return true
		{"", true},
		{"0", true},
		{"27", true},
		{"0;1", true},  // 0 resets
		{"1;27", true}, // 27 resets
		{" ;0", true},  // empty param (after trim) = reset
	}
	for _, tc := range cases {
		t.Run(tc.params, func(t *testing.T) {
			got := sgrClearsReverse(tc.params)
			if got != tc.want {
				t.Fatalf("sgrClearsReverse(%q) = %v, want %v", tc.params, got, tc.want)
			}
		})
	}
}
...
// detectSideContentCols: content that has pipes only on the left side of the
// gutter, with no pipes after/right of gutterCol, returns (-1, -1).
func TestDetectSideContentCols_NoPipesRightOfGutter(t *testing.T) {
	pad := func(n int) string { return strings.Repeat(" ", n) }
	// Build lines with pipes at cols 0, 5, 30 (gutter), but nothing to the right.
	line := "│" + "  1 " + "│" + pad(24) + "│"
	left, right := detectSideContentCols(line+"\n"+line, 30)
	if left != -1 || right != -1 {
		t.Fatalf("expected (-1, -1) when no pipes right of gutter, got (%d, %d)", left, right)
	}
}

// detectSideContentCols: content with pipes on the left but no pipes on the
// left side between col>0 and col<gutterCol — leftCounts stays empty, returns (-1,-1).
func TestDetectSideContentCols_NoPipesLeftOfGutter(t *testing.T) {
	pad := func(n int) string { return strings.Repeat(" ", n) }
	// Only col-0 '│' and gutter '│', no leftPostLN for lines with <4 pipes
	line := "│" + pad(29) + "│" + pad(29) + "│"
	left, right := detectSideContentCols(line+"\n"+line, 30)
	if left != -1 || right != -1 {
		t.Fatalf("expected (-1, -1) when no left post-LN pipe, got (%d, %d)", left, right)
	}
}
